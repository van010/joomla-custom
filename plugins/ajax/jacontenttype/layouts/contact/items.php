	...
	<fieldset class="filter clearfix">
		<div class="btn-toolbar">
			<div class="btn-group pull-left">
				...
				<input type="text" name="filter[search]" id="filter_search" value="<?php echo $this->escape($state->get('filter.search')); ?>" size="30" title="<?php echo JText::_('COM_CONTACT_FILTER_SEARCH_DESC'); ?>" />
			</div>
			<div class="btn-group pull-left">
		...
		<div class="filters pull-left">
			...
			<?php if ($state->get('filter.forcedLanguage')) : ?>
				...
				<input type="hidden" name="filter[language]" value="<?php echo $this->escape($state->get('filter.language')); ?>" />
			<?php else : ?>
				<select name="filter[category_id]" class="input-medium" onchange="this.form.submit()">
					<option value=""><?php echo JText::_('JOPTION_SELECT_CATEGORY');?></option>
					<?php echo JHtml::_('select.options', JHtml::_('category.options', 'com_contact'), 'value', 'text', $state->get('filter.category_id'));?>
				</select>
				<select name="filter[language]" class="input-medium" onchange="this.form.submit()">
			...
			<tr class="row<?php echo $i % 2; ?>">
				<td>
					<?php if (version_compare(JVERSION, '4.0', 'ge')): ?>
						<a class="pointer button-select" 
							href="#" 
							data-custom-value="<?php echo $item->id; ?>" 
							data-custom-name="<?php echo $this->escape($item->name); ?>" 
							data-user-value="<?php echo $item->id; ?>" 
							data-user-name="<?php echo $this->escape($item->name); ?>" 
							data-user-field="<?php echo $field_id; ?>">
