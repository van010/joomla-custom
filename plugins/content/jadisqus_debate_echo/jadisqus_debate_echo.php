...
class plgContentJADisqus_debate_echo extends JPlugin
{
    ...
    function __construct(&$subject, $config)
    {
        ...
        if(JFactory::getApplication()->input->getCmd('style','') !== 'raw'){
    ...
    {
        return ($article->params && !empty($article->params->get('article_layout',''))) ? true : false;
    }
    ...
    */
    ...
    function stylesheet($plugin)
    {
        $app = JFactory::getApplication();
        $doc = JFactory::getDocument();
        $doc->addStylesheet('plugins/' . $plugin->type . '/' . $plugin->name . '/asset/style.css');
        if (is_file(JPATH_SITE . '/templates/' . $app->getTemplate() . '/css/' . $plugin->name . ".css"))
            $doc->addStylesheet('templates/' . $app->getTemplate() . '/css/' . $plugin->name . ".css");
    }
