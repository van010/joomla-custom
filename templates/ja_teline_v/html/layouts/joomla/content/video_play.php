...
if ($ctm_source == 'other') {
	...
	if ($url) {
		...
		echo '<iframe class="ja-video" width="'.$width.'" height="'.$height.'" src="'.$url.'" style="border:none" allowfullscreen allow="autoplay; encrypted-media"></iframe>';
	}
	...
	if($src) {
		if($context == 'list' || $context == 'featured'):
			$thubnail = $params->get('ctm_thumbnail');
			echo '<video src="'.JUri::root(true) . '/' . $src.'" poster="'.htmlspecialchars(JUri::root(true) . '/' . $thubnail).'" controls ></video>';
		else:
			JHtml::_('jquery.framework');
